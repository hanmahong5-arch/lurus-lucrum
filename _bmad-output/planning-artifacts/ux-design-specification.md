...
status: complete
inputDocuments: [prd-gushen.md, product-brief.md, project-context.md, gushen-web/docs/DESIGN_SYSTEM.md]
date: 2026-02-02
author: Anita
project: lurus-gushen
---

# UX Design Specification lurus-gushen
...
GuShen (股神) 是一个 AI 量化交易决策平台。其核心不是"提供量化工具"，而是"帮助用户做出更好的投资决策"。平台通过自然语言策略构建、金融级回测引擎、多视角 AI 顾问和自动策略发现，让非程序员也能进行专业级的策略验证。
...
**GuShen 借鉴点**：K线图交互标准、指标叠加模式、策略社区浏览体验
...
**GuShen 借鉴点**：可折叠面板布局、命令面板快捷操作、底部状态栏（保存状态/数据源/工作流步骤）
...
**GuShen 借鉴点**：AI 建议内嵌在策略编辑器中、回测结果旁的 AI 点评面板、"一键应用"参数调整建议
...
**GuShen 借鉴点**：空状态引导设计、Slash 命令式策略输入、面包屑式工作流导航

#### 聚宽 (JoinQuant) — 中国量化竞品

**优势**：成熟策略社区、完整回测框架、中文生态
**劣势（GuShen差异化方向）**：
- 必须会 Python 才能使用 → GuShen：自然语言输入
- 回测结果展示专业但不友好 → GuShen：策略评分 + 分层披露
- 无 AI 投资顾问 → GuShen：11 Agent 多视角分析
- 无工作流引导 → GuShen：4步工作流导航
- 界面风格传统 → GuShen：现代暗色驾驶舱
...
GuShen 采用 TailwindCSS + Radix UI 作为基础层，叠加完整的自定义设计令牌体系。这不是简单的"套模板"，而是在无样式原语之上构建专属金融驾驶舱视觉语言。
...
这是 GuShen 的 "Tinder Swipe" — 用户会这样向朋友介绍这个产品。所有其他功能（多股验证、AI 顾问、策略发现）都是这个核心循环的扩展和深化。
...
**CSS 变量命名规范**：`--gushen-{category}-{name}-{variant}`
...
| 类别 | 令牌 | 色值 | 用途 | 状态 |
|------|------|------|------|------|
| 背景 | `--gushen-bg-void` | #09090b | 全局底色 | ✅ 已实现 |
| 背景 | `--gushen-bg-void-soft` | #121218 | 日间暗色变体 | 📋 P2 预留 |
| 背景 | `--gushen-bg-surface` | #18181b | 面板/卡片 (Level 1) | ✅ 已实现 |
| 背景 | `--gushen-bg-surface-elevated` | #1f1f23 | 内嵌卡片 (Level 2) | 📋 需新增 |
| 背景 | `--gushen-bg-surface-hover` | #27272a | 悬停/弹出层 (Level 3) | ✅ 已实现 |
| 背景 | `--gushen-bg-surface-modal` | #2d2d33 | Modal/Dialog (Level 4) | 📋 需新增 |
| 语义 | `--gushen-color-primary` | #3b82f6 | 主操作色（信任蓝） | ✅ 已实现 |
| 语义 | `--gushen-color-accent` | #eab308 | 警告/强调（金色） | ✅ 已实现 |
| 市场 | `--gushen-color-profit` | #ef4444 | 上涨（中国市场红涨） | ✅ 已实现 |
| 市场 | `--gushen-color-loss` | #22c55e | 下跌（中国市场绿跌） | ✅ 已实现 |
| 市场 | `--gushen-color-profit-muted` | #fca5a5 | 上涨低饱和（长时间观看） | 📋 需新增 |
| 市场 | `--gushen-color-loss-muted` | #86efac | 下跌低饱和（长时间观看） | 📋 需新增 |

**策略评分色（三重编码：字母 + 描述 + 图标）**

| 等级 | 令牌 | 色值 | 描述文字 | 图标 | 设计理由 | 状态 |
|------|------|------|---------|------|---------|------|
| S | `--gushen-color-score-s` | #fbbf24 | 卓越 | ★★★ | 金色光辉，最高成就 | 📋 需新增 |
| A | `--gushen-color-score-a` | #22d3ee | 优秀 | ★★ | 青色（与B级蓝拉开色相） | 📋 需新增 |
| B | `--gushen-color-score-b` | #3b82f6 | 良好 | ★ | 稳健蓝 | 📋 需新增 |
| C | `--gushen-color-score-c` | #6b7280 | 一般 | ○ | 中性灰 | 📋 需新增 |
| D | `--gushen-color-score-d` | #fb923c | 需改进 | ✕ | 橙色（与金色和红色拉开） | 📋 需新增 |
...
| 来源 | 令牌 | 色值 | 含义 | 状态 |
|------|------|------|------|------|
| 数据库 | `--gushen-color-source-db` | #3b82f6 | 真实历史数据 | 📋 需新增 |
| API | `--gushen-color-source-api` | #eab308 | 实时拉取 | 📋 需新增 |
| 模拟 | `--gushen-color-source-sim` | #6b7280 | 模拟数据（需警示） | 📋 需新增 |

**功能扩展色**

| 用途 | 令牌 | 色值 | 状态 |
|------|------|------|------|
| 工作流活跃 | `--gushen-color-step-active` | #3b82f6 | 📋 需新增 |
| 工作流完成 | `--gushen-color-step-done` | #22c55e | 📋 需新增 |
| 工作流待执行 | `--gushen-color-step-pending` | #4b5563 | 📋 需新增 |
| AI 标记色 | `--gushen-color-ai` | #a78bfa | 📋 需新增 |
| AI 背景 | `--gushen-bg-ai` | rgba(167,139,250,0.10) 最低；bg-surface 上 15% | 📋 需新增 |
| AI 边框 | `--gushen-border-ai` | rgba(167,139,250,0.20) | 📋 需新增 |
| AI 左侧强标记 | `border-left` | 2px solid #a78bfa | 📋 需新增 |
| 缓存命中 | `--gushen-color-cache` | #6b7280 | 📋 需新增 |
| 警告横幅 | `--gushen-color-banner-warn` | #92400e | 📋 需新增 |
| 状态灯-就绪 | `--gushen-color-status-ready` | #22c55e (● 绿灯) | 📋 需新增 |
| 状态灯-警告 | `--gushen-color-status-warn` | #eab308 (● 黄灯) | 📋 需新增 |
| 状态灯-阻断 | `--gushen-color-status-block` | #ef4444 (● 红灯) | 📋 需新增 |
...
| 令牌 | 值 | Tailwind | 用途 |
|------|-----|---------|------|
| `--gushen-space-xs` | 4px | p-1 | Badge 内边距 |
| `--gushen-space-sm` | 8px | p-2 | 图标与文字间距 |
| `--gushen-space-md` | 12px | p-3 | 组件内部间距 |
| `--gushen-space-lg` | 16px | p-4 | 面板内边距（默认） |
| `--gushen-space-xl` | 24px | p-6 | 主区域间距 |
| `--gushen-space-2xl` | 32px | p-8 | 页面级分隔 |
...
**回测进度条**：`--gushen-color-primary` 蓝色填充，bg-surface-elevated 底色，右侧百分比（Data MD）。

### AI Visual Language

| 元素 | 样式 | 用途 |
|------|------|------|
| 背景 | `--gushen-bg-ai`: rgba(167,139,250,0.10)；bg-surface 上 15% | AI 区域 |
| 左侧强标记 | `border-left: 2px solid var(--gushen-color-ai)` | AI 卡片左边框 |
| 标记色 | `--gushen-color-ai`: #a78bfa | 图标、标签 |
| 边框 | `--gushen-border-ai`: rgba(167,139,250,0.20) | AI 卡片边框 |
...
- 正面：`bg-surface-elevated` + `--gushen-color-profit-muted`（"高胜率"）
- 负面：`bg-surface-elevated` + `--gushen-color-loss-muted`（"高回撤"）
...
**对比模式**（迭代优化场景）
- 左右分栏：修改前 vs 修改后
- 提升项：`--gushen-color-profit` 标色 + ↑ 箭头
- 下降项：`--gushen-color-loss` 标色 + ↓ 箭头
...
- 主按钮：`btn-tactile` + `--gushen-color-primary` 填充
...
| 类型 | 左侧标记 | 图标 | 背景 | 用途 |
|------|---------|------|------|------|
| 成功 | 2px `--gushen-color-step-done` | ✓ | bg-surface-elevated | 保存成功、回测完成 |
| 错误 | 2px `--gushen-color-status-block` | ✕ | bg-surface-elevated | 操作失败、验证错误 |
| 警告 | 2px `--gushen-color-status-warn` | ⚠ | bg-surface-elevated | 模拟数据提醒、缓存过期 |
| AI 通知 | 2px `--gushen-color-ai` | AI图标 | --gushen-bg-ai | AI 建议就绪、顾问回复 |
...
| 断点 | 变量 | 值 | 布局 |
|------|------|---|------|
| 桌面 | `--gushen-breakpoint-desktop` | 1280px | 多面板并排/Split |
| 平板 | `--gushen-breakpoint-tablet` | 768px | 单面板+Tab 切换 |
| 移动 | < 768px | — | 卡片流+折叠+只读 |
...
GuShen 是专业量化交易工具，核心操作（策略编辑、回测分析、多股验证）需要大屏空间。桌面优先，向下渐进削减。
...
```css
:root {
  --gushen-breakpoint-sm: 640px;
  --gushen-breakpoint-md: 768px;
  --gushen-breakpoint-lg: 1024px;
  --gushen-breakpoint-xl: 1280px;
  --gushen-breakpoint-2xl: 1536px;
}
```
...
@media (prefers-reduced-motion: reduce) {
  *, *::before, *::after {
  ...
  .shimmer { animation: none; background: var(--gushen-bg-surface-hover); }
...
GuShen 只有暗色模式，明确忽略 `prefers-color-scheme: light`。在设置页说明："GuShen 专为暗色环境设计"。
...
### GuShen Data Protocol (QUANTAXIS QIFI 启发)

数据对象内嵌响应式展示和无障碍元信息：

```typescript
interface GushenDataProtocol<T = unknown> {
