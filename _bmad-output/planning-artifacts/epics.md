...
- 评分色: `--gushen-color-score-s` (#fbbf24), `score-a` (#22d3ee), `score-b` (#3b82f6), `score-c` (#6b7280), `score-d` (#fb923c)
- 数据源色: `--gushen-color-source-db` (#3b82f6), `source-api` (#eab308), `source-sim` (#6b7280)
- AI 色: `--gushen-color-ai` (#a78bfa), `--gushen-bg-ai` (rgba(167,139,250,0.10)), `--gushen-border-ai` (rgba(167,139,250,0.20))
- 步骤色: `--gushen-color-step-active` (#3b82f6), `step-done` (#22c55e), `step-pending` (#4b5563)
- 状态灯色: `--gushen-color-status-ready` (#22c55e), `status-warn` (#eab308), `status-block` (#ef4444)
- 背景层级: `--gushen-bg-surface-elevated` (#1f1f23), `--gushen-bg-surface-modal` (#2d2d33)
...
- 1-2 个操作按钮 (主按钮 `btn-tactile` + `--gushen-color-primary`, 可选次按钮 ghost)
...
- 黄色横幅 (`--gushen-color-banner-warn` 背景)
- 文字: "当前使用模拟数据，结果仅供参考"
...
- 当前阶段显示百分比进度
- 已完成阶段显示 ✓
**And** 进度条使用 `--gushen-color-primary` 填充色，`bg-surface-elevated` 底色
...
**And** 背景使用 `bg-surface-elevated`，左侧 2px `--gushen-color-status-block` 红色标记
...
**When** AI 元素渲染
**Then** 使用统一的 AI 视觉语言:
- 背景: `--gushen-bg-ai` (rgba(167,139,250,0.10))
- 左侧强标记: `border-left: 2px solid var(--gushen-color-ai)` (#a78bfa)
- 边框: `--gushen-border-ai`
