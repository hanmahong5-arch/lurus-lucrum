...
- [x] Pod镜像: docker.io/library/lucrum-web:v18
...
- 自定义 storage 实现，使用 `lucrum:{userId}:{key}` 格式
- 多标签页同步使用用户范围的键
...
2. ✅ 用户范围的localStorage键格式 `lucrum:{userId}:{key}`
3. ✅ Dashboard头部显示账户状态
4. ✅ 角色徽章（免费版/标准版/专业版）
5. ✅ 用户下拉菜单（设置、策略、登出）
...
- popular_strategies: added cache_key, author_id, avg_return, usage_count columns + unique index
...
Verification: GET /api/advisor/chat → 200 OK; GET /api/agent/scanner → 405 (POST-only route registered).
