 ...
 * Follows Lucrum dark-first design system (bg-surface, font-mono tabular-nums).
 */
...
export function AccountPanel({ overview }: AccountPanelProps) {
  ...
  const topupLink = `${topup_url ?? ''}?redirect=${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '/')}&from=lurus-lucrum`;
