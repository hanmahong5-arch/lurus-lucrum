 ...
 *
 * Connects to lucrum-web /api/strategy/* endpoints.
 */
...
export function useGenerateStrategy() {
  return useMutation({
    mutationFn: async (request: GenerateRequest) => {
      const { data } = await lucrumApi.post<GenerateResponse>(
        "/strategy/generate",
        request,
      );
...
export function useExplainStrategy() {
  return useMutation({
    mutationFn: async (strategyCode: string) => {
      const { data } = await lucrumApi.post<ExplainResponse>(
        "/strategy/optimize",
...
export function useSuggestParams() {
  return useMutation({
    mutationFn: async (params: {
      ...
      const { data } = await lucrumApi.post<SuggestParamsResponse>(
        "/strategy/optimize",
        {
          action: "suggest_params",
          ...params,
