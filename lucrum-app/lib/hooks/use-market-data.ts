/**
 * Market Data Hooks (adapted from lucrum-web)
 *
 * Uses React Query + axios. Endpoints call lucrum-web API routes.
 */

import { useQuery } from "@tanstack/react-query";
import { lucrumApi } from "@/lib/api/client";
...
export function useStockList(options: StockListOptions = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await lucrumApi.get<PaginatedResponse<StockListItem>>(
        `/stocks/list?${params}`,
      );
      return data;
    },
...
export function useStockSearch(query: string, limit = 10) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await lucrumApi.get<SearchResponse>(
...
export function useStockQuote(symbol: string, refreshInterval = 0) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await lucrumApi.get<MarketApiResponse<StockQuote>>(
...
export function useBatchQuotes(symbols: string[], refreshInterval = 0) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await lucrumApi.get<MarketApiResponse<Record<string, StockQuote>>>(
...
export function useKLineData(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await lucrumApi.get<MarketApiResponse<KLineData[]>>(
...
export function useMajorIndices(refreshInterval = 30_000) {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const { data } = await lucrumApi.get<MarketApiResponse<IndexQuote[]>>(
          "/market/indices",
        );
...
export function useCapitalFlow(symbol: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await lucrumApi.get<MarketApiResponse<Record<string, unknown>>>(
...
export function useStockDateRange(symbol: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await lucrumApi.get<MarketApiResponse<{
        symbol: string;
        minDate: string;
        maxDate: string;
        dataPoints: number;
