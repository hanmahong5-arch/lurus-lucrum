...
/**
 * Billing API Integration Test Page
 *
 * This page tests the billing API integration with lurus-api.
...
export default function BillingTestPage() {
  ...
  const testUnauthorizedAccess = async () => {
    ...
    try {
      // Try to access without session (should fail)
      const response = await fetch("/api/lurus/billing/plans", {
        credentials: "omit", // Don't send session cookie
      });
      const data = await response.json();
  ...
  const runAllTests = async () => {
    ...
    // Skip create test to avoid creating unnecessary data
